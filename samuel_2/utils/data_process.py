...
class DataProcess:
    ...
    def __init__(self, year_min=2017, year_max=2100, limit_to_ambo=False):
        """
    ...
    def create_ml_data(self):
        ...
        # Calculate onset to thrombectomy (return -10 for no thrombectomy)
        def f(row):
            if row['arrival_to_thrombectomy_time'] >= 0:
                return row['arrival_to_thrombectomy_time'] + row['onset_to_arrival_time']
            else:
                return 99999
